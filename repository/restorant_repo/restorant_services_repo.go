...
func AddService(service restorantmodels.RestaurantService) (string, error) {
	...
	query := `
        ...
        (service_id, restaurant_id, category_id, service_name, service_description, service_price, items)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `
...
func GetServiceWithImages(serviceID string) (restorantmodels.RestaurantService, []string, error) {
	...
	query := `
        SELECT service_id, restaurant_id, category_id, service_name, service_description, service_price, items
        FROM Restaurant_Services
        WHERE service_id = ?
    `
	row := config.DB.QueryRow(query, serviceID)
	err := row.Scan(&service.ServiceID, &service.RestaurantID, &service.CategoryId,
		&service.ServiceName, &service.ServiceDescription, &service.ServicePrice, &service.Items)
	if err != nil {
...
func GetAllServicesWithImages(restaurantID string) ([]restorantmodels.RestaurantService, error) {
	...
	query := `SELECT service_id, restaurant_id, category_id, service_name, service_description, service_price, items, created_at, updated_at
              FROM Restaurant_Services
              WHERE restaurant_id = ?`
